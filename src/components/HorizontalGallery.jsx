...
const HorizontalGallery = ({ panels }) => {
  ...
  if (reduceMotion) {
    return (
      <section className="relative bg-soft py-16 overflow-hidden">
        <GalleryBackdrop />
...
const Panel = ({ panel, index, total }) => {
  const Icon = panel.icon || DEFAULT_ICONS[index % DEFAULT_ICONS.length];
  const accent = PANEL_ACCENTS[index % PANEL_ACCENTS.length];
  const isTeal = accent === "teal";

  return (
    <div className="container-px w-full grid md:grid-cols-2 gap-8 md:gap-16 items-center relative">
      <div className="order-2 md:order-1 relative">
        ...
        <div className={`mt-5 pl-5 border-l max-w-lg ${isTeal ? "border-teal/60" : "border-sand/60"}`}>
          <p className="text-slate-700 leading-relaxed text-[15px] font-normal">{panel.description}</p>
        </div>
